...
// AdaptiveTCPOverrideHook and AdaptiveTCPFailureHook are downstream extension
// points used by FlClash Adaptive. Nil preserves upstream behavior exactly.
var (
	AdaptiveTCPOverrideHook func(metadata *C.Metadata, proxy C.Proxy, rule C.Rule) (C.Proxy, bool)
	AdaptiveTCPFailureHook  func(metadata *C.Metadata, original C.Proxy, selected C.Proxy, rule C.Rule, err error, overridden bool)
)
...
func handleTCPConn(connCtx C.ConnContext) {
	...
	originalProxy := proxy
	adaptiveOverridden := false
	if AdaptiveTCPOverrideHook != nil {
		if selected, ok := AdaptiveTCPOverrideHook(metadata, proxy, rule); ok && selected != nil {
			proxy = selected
			adaptiveOverridden = true
		}
	}
	...
	if err != nil {
		if AdaptiveTCPFailureHook != nil {
			AdaptiveTCPFailureHook(metadata, originalProxy, proxy, rule, err, adaptiveOverridden)
		}
		return
	}
