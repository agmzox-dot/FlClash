...
func (tt *tcpTracker) Read(b []byte) (int, error) {
	...
	if download > 0 && DefaultRequestTrafficNotify != nil && DefaultRequestTrafficNotify(tt, 0, download) {
		_ = tt.Conn.Close()
	}
	return n, err
}
...
func (tt *tcpTracker) Write(b []byte) (int, error) {
	...
	if upload > 0 && DefaultRequestTrafficNotify != nil && DefaultRequestTrafficNotify(tt, upload, 0) {
		_ = tt.Conn.Close()
	}
	return n, err
}
...
func NewTCPTracker(conn C.Conn, manager *Manager, metadata *C.Metadata, rule C.Rule, uploadTotal int64, downloadTotal int64, pushToManager bool) *tcpTracker {
	...
	manager.Join(tt)
	if (uploadTotal > 0 || downloadTotal > 0) && DefaultRequestTrafficNotify != nil && DefaultRequestTrafficNotify(tt, uploadTotal, downloadTotal) {
		_ = tt.Conn.Close()
	}
	return tt
}
