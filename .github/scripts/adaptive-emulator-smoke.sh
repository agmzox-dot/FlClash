...
adb_install_candidate() {
  ...
  while (( attempts <= 2 )); do
    ...
    : > "$output_file"
    status=''
    printf 'starting streaming adb install attempt %s (timeout %ss)\n' "$attempts" "$apk_install_timeout_seconds" >> "$error_file"
    if bounded_timeout "${apk_install_timeout_seconds}s" adb install --streaming --no-incremental -r -d --abi x86_64 "$apk" >> "$output_file" 2>&1; then
      if grep -Eq '(^|[[:space:]])Success([[:space:]]|$)' "$output_file"; then
        printf 'streaming adb install attempt %s succeeded:\n' "$attempts" >> "$error_file"
    ...
    printf 'streaming adb install attempt %s failed (exit %s):\n' "$attempts" "$status" >> "$error_file"
    cat "$output_file" >> "$error_file"
    detail="$(tr '\r\n' ' ' < "$output_file" | tr -s ' ')"
    if (( ${#detail} > 240 )); then
      detail="${detail: -240}"
    ...
    if (( status == 124 || status == 137 )); then
      record_error "streaming APK install attempt $attempts/2 timed out after ${apk_install_timeout_seconds} seconds"
    elif [[ -n "$detail" ]]; then
      record_error "streaming APK install attempt $attempts/2 failed: $detail"
    else
      record_error "streaming APK install attempt $attempts/2 failed with exit $status"
    fi
    attempts=$((attempts + 1))
    if (( attempts <= 2 )); then
      sleep 3
...
verify_apk_identity_and_signatures() {
  ...
  printf 'upgrade_check: emulator-side pm install -r is performed twice on the same certificate in this run\n' >> "$signature_file"
